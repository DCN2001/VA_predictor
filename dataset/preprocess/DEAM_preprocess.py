...
def load_id_list(file_path):
    with open(file_path, "r") as f:
        return [f"{int(line.strip())}.wav" for line in f if line.strip()]

def DEAM_preprocess(args):
    #Setting
    datapath = args.datapath
    label_df1 = pd.read_csv(os.path.join(datapath,"DEAM_Annotations/annotations/annotations averaged per song/song_level/static_annotations_averaged_songs_1_2000.csv"))
    label_df2 = pd.read_csv(os.path.join(datapath,"DEAM_Annotations/annotations/annotations averaged per song/song_level/static_annotations_averaged_songs_2000_2058.csv"))
    ...
    #Split dataset
    train_path = "./DEAM_split/train.txt"
    valid_path = "./DEAM_split/valid.txt"
    test_path = "./DEAM_split/test.txt"
    ...
    for file in tqdm(train_files):
        #Get label
        file_id = int(file.replace(".wav",""))
        if file_id in label_df1['song_id'].values:
            row = label_df1[label_df1['song_id'] == file_id].iloc[0]
            valence_mean = row[' valence_mean']
            arousal_mean = row[' arousal_mean']
        else:
            row = label_df2[label_df2['song_id'] == file_id].iloc[0]
            valence_mean = row[' valence_mean']
            arousal_mean = row[' arousal_mean']
        ...
        #Read audio
        song_path = os.path.join(datapath,"DEAM_audio","wav_audio",file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datapath', type=str, default='/mnt/gestalt/home/dcn2001/DEAM', help="Path of DEAM dataset")
    parser.add_argument('--destpath', type=str, default='/mnt/gestalt/home/dcn2001/DEAM_preprocessed', help="Destination path for saving the feature")
    args = parser.parse_args()
    
    DEAM_preprocess(args)